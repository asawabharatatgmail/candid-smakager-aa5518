...
const BranchAdminDashboard: React.FC = () => {
  const { setActiveView, filteredStudents, filteredTeachers, filteredClasses } = useAppContext();
  const branchLinks = NAV_LINKS[UserRole.BranchAdmin].filter(l => l.key !== 'dashboard');

  const stats = [
    { label: 'Students',  value: filteredStudents.length,  icon: 'ri-graduation-cap-line', color: 'bg-indigo-100 text-indigo-600',  view: 'students' },
    { label: 'Teachers',  value: filteredTeachers.length,  icon: 'ri-user-3-line',         color: 'bg-emerald-100 text-emerald-600', view: 'teachers' },
    { label: 'Classes',   value: filteredClasses.length,   icon: 'ri-book-2-line',         color: 'bg-violet-100 text-violet-600',  view: 'classes-subjects' },
  ];

  return (
    <div className="space-y-8">
      <div>
        <h1 className="page-title text-2xl">Branch Dashboard</h1>
        <p className="page-sub">Manage students, teachers, and operations for your branch.</p>
      </div>
      ...
      <PersonalizedInsights role={UserRole.BranchAdmin} />

      <div>
        <h2 className="section-head">Branch Controls</h2>
